...
探索代码库、查找依赖、追踪调用链、影响分析、找死代码等结构性查询，**优先用 `codebase-memory` 知识图谱**（MCP 工具前缀 `mcp__codebase-memory-mcp__`），不可用或项目未索引时才退回 Bash 里的 `rg`/`fd`（见 CLAUDE.md「Shell 命令」节；带 Grep/Glob 工具的子 agent 也可用内置工具）。具体工具映射与工作流见 `~/.claude/skills/codebase-memory/SKILL.md`。
