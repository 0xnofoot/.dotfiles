#!/usr/bin/env python3
"""
从 vscode/default-keybindings/ 目录中的默认快捷键 JSON 生成完整禁用列表，更新 keybindings.json。

用法:
  python3 generate-disabled-defaults.py                    # 读取 vscode/default-keybindings/*.json
...
导出默认快捷键:
  ...
  全选复制，保存到 vscode/default-keybindings/<编辑器名>.json（如 vscode.json、cursor.json）

脚本自动读取 vscode/default-keybindings/ 下所有 .json 文件，合并去重后生成禁用条目。
命令行额外传入的文件也会一并合并。
"""
...
DEFAULTS_DIR = VSCODE_DIR / "default-keybindings"
...
def collect_input_files(extra_args: list[str]) -> list[Path]:
    """收集输入文件：default-keybindings/ 目录 + 命令行参数"""
...
def main():
    ...
    if check_mode:
        if new_content != content:
            print(f"keybindings.json 中的禁用列表与 default-keybindings/ 不一致")
